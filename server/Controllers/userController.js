...
const registerUser = async (req, res) => {
  ...
  try {
    ...
    user = new userModel({ first_name, last_name, company_name, email, password, emailToken: crypto.randomBytes(64).toString("hex") });
    ...
    res.status(200).json({ _id: user._id, first_name, last_name, company_name, email, token });
  } catch (error) {
    console.log(error);
...
const loginUser = async (req, res) => {
  ...
  try {
    ...
    res.status(200).json({ _id: user._id, first_name: user.first_name, last_name: user.last_name, company_name: user.company_name, email, token });
  } catch (error) {
    console.log(error);
    res.status(500).json(error);
  }
